...
class TestSCCNNLayer:
    ...
    def test_forward(self):
        ...
        sccnn = SCCNNLayer(in_channels=channels_all, out_channels=channels_all, conv_order=2, sc_order=2, aggr_norm=False, update_func=None)
        output = sccnn.forward(x_all, incidence_all, laplacian_all)
        y_0, y_1, y_2 = output
        assert y_0.shape == (n_nodes, channels)
        assert y_1.shape == (n_nodes, channels)
        assert y_2.shape == (n_nodes, channels)


        # Without aggregation norm, with update function 
        sccnn = SCCNNLayer(in_channels=channels_all, out_channels=channels_all, conv_order=2, sc_order=2, aggr_norm=False, update_func='sigmoid')
        output = sccnn.forward(x_all, incidence_all, laplacian_all)
        y_0, y_1, y_2 = output
        assert y_0.shape == (n_nodes, channels)
        assert y_1.shape == (n_nodes, channels)
        ...
        sccnn = SCCNNLayer(in_channels=channels_all, out_channels=channels_all, conv_order=2, sc_order=2, aggr_norm=True, update_func='sigmoid')
        output = sccnn.forward(x_all, incidence_all, laplacian_all)
        y_0, y_1, y_2 = output
        assert y_0.shape == (n_nodes, channels)
        assert y_1.shape == (n_nodes, channels)
